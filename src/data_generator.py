...
def generate_energy_data(
    ...
    price_noise = np.random.normal(0, 8, len(df))
    ...
    df["electricity_price"] = df["electricity_price"].clip(lower=5)
    ...
    df["historical_energy_usuage"] = (
            load_effect
            + temperature_effect
            + energy_noise
    )

    df["historical_energy_usuage"] = df["historical_energy_usuage"].clip(lower=20)
